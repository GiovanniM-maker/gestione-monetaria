...
function Barra({ className = '' }: { className?: string }) {
  return <span className={`block rounded-full bg-s3 ${className}`} />;
}
...
export function ScheletroTestata() {
  return (
    <div className="animate-pulse space-y-4" aria-hidden="true">
      ...
      <div className="scheda space-y-4 p-4">
        {Array.from({ length: 4 }, (_, i) => (
          <div key={i} className="flex items-center gap-3">
            <Barra className="size-2.5 shrink-0" />
...
export function ScheletroCiambella() {
  return (
    <div
      ...
      aria-hidden="true"
    >
      <span className="size-40 shrink-0 rounded-full border-[16px] border-(--s3)" />
...
export function ScheletroElenco({ righe = 6 }: { righe?: number }) {
  return (
    <div className="scheda animate-pulse px-4" aria-hidden="true">
      <ul className="elenco">
        {Array.from({ length: righe }, (_, i) => (
          <li key={i} className="flex min-h-12 items-center gap-3">
...
export function ScheletroCoppia() {
  return (
    <div className="grid animate-pulse grid-cols-2 gap-3" aria-hidden="true">
      {[0, 1].map((i) => (
        <div key={i} className="scheda space-y-2.5 p-4">
